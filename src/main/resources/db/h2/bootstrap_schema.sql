...
CREATE TABLE COMMENT (
    ...
    SOURCE_ID INT NOT NULL,         -- ID OF SOURCE TYPE BEING COMMENTED ON
    ITEM_ID INT NOT NULL,           -- ID OF SOURCE ITEM BEING COMMENTED ON
    AUTHOR_ID VARCHAR(40) NOT NULL, -- ID OF USER WHO COMMENTED
    CONTENT VARCHAR(MAX) DEFAULT NULL,
    UPVOTE_COUNT INT NOT NULL DEFAULT 0,
    CONSTRAINT FK_COMMENT_SOURCE_ID_SOURCE_ID FOREIGN KEY (SOURCE_ID) REFERENCES SOURCE(ID) ON DELETE CASCADE
);
-- MODELS A VOTE ON A SOURCE
CREATE TABLE VOTE (
    ID INT DEFAULT VOTE_SEQUENCE.NEXTVAL PRIMARY KEY,
    SOURCE_ID INT NOT NULL,         -- ID OF SOURCE TYPE BEING VOTED ON
    ITEM_ID INT NOT NULL,           -- ID OF SOURCE ITEM BEING VOTED ON
    AUTHOR_ID VARCHAR(40) NOT NULL, -- ID OF USER WHO VOTED
    ACTION VARCHAR(63) NOT NULL,    -- EITHER UPVOTE OR DOWNVOTE
    CONSTRAINT U_VOTE_AUTHOR_ID_SOURCE_ID_ITEM_ID UNIQUE (AUTHOR_ID, SOURCE_ID, ITEM_ID),
    CONSTRAINT FK_VOTE_SOURCE_ID_SOURCE_ID FOREIGN KEY (SOURCE_ID) REFERENCES SOURCE(ID) ON DELETE CASCADE
);